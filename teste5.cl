...
class Main inherits IO {
   ...
   main() : Object {
	   {
		  avar <- (new A);
		  avar <- (new A).set_var(10); -- Hardcode the starting number to 10
		  class_type(avar);

		  char <- "a";                 -- Force the "add" operation
		  a_var <- (new A).set_var(5); -- Hardcode the number to add
		  avar <- (new B).method2(avar.value(), a_var.value());
		  print(avar);                 -- Will output 15
